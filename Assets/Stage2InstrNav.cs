...
public class Stage2InstrNav : MonoBehaviour {
	private Text[] instr; 
	private int current; 
	...
	public Stage3Controller stage3Controller; 
//	public GameObject instructionLarge; 
	...
	void Start () {
		...
		if (isCuddleMenu) {
			...
			collided = false; 
			toward = Quaternion.Euler (0, 359f, 0); 
			childMove = true; 
	...
	void Update () {
		...
		if (isCuddleMenu) {
			...
			if (current == numInstr - 1) {
				...
				if ((child.transform.rotation.eulerAngles.y >= 358.0f || child.transform.rotation.eulerAngles.y <= 0.1f) && childTurn1) {
					Debug.Log ("stop turn"); 
					childTurn1 = false; 
					childMove = true; 
				}
				...
				if (Vector3.Distance(childOrigin, child.transform.position) <= 0.05f && childMove) {
	...
	public void NextInstr() {
		if (acceptNext) {
			if (current < numInstr - 1) {
				...
				if (SceneManager.GetActiveScene ().name.Equals ("Stage2V2") || SceneManager.GetActiveScene ().name.Equals ("Stage4V2")) {
